...
func TestClientBreakerMan(t *testing.T) {
	...
	t.Run("dispatches Man synchronously", func(t *testing.T) {
		manRes, err := b.Man(context.Background(), new(ManRequest))
...
func TestClientBreakerHandle(t *testing.T) {
	req := &HandleRequest{
		Event: &termrpc.Event{Char: '$', Type: termrpc.Event_TypeKey},
		Draw:  &DrawRequest{},
	}
...
func assertDrawResponse(t *testing.T, res *HandleResponse, strCopy string) {
	require.NotNil(t, res)
	require.NotNil(t, res.Draw)
	if assert.NotNil(t, res.Draw.Cursor) {
		assert.NotNil(t, res.Draw.Cursor.Position)
	}
	str, width, height := DrawResponseToTermString(res.Draw)
	if strCopy == "" {
		return
	}
	expected := component.NewStringWithConfig(strCopy,
