...
var _ textapi.CommandHandler = (*declarationHandler)(nil)

func TestDeclarationHandler(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "single declaration navigates directly",
			result: semanticapi.LocationResult{
		...
		{name: "no declarations", wantNotifyErr: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cmd := textapi.Command{Name: "declaration", URI: uri}
			if !tt.nilResource {
