...
func TestChatTemplateOptions_Defaults(t *testing.T) {
	...
	opts := makeChatTemplateOptions("", nil, llmapi.ReasoningEffortHigh, true, "", nil, nil)
...
func TestChatTemplateOptions_WithToolsKeepsThinkingByDefault(t *testing.T) {
	...
	opts := makeChatTemplateOptions("", []Tool{{Name: "list_dir"}}, llmapi.ReasoningEffortHigh, true, "", nil, nil)
	if !opts.EnableThinking {
...
func TestChatTemplateOptions_MinimalEffortDisablesThinking(t *testing.T) {
	...
	opts := makeChatTemplateOptions("", nil, llmapi.ReasoningEffortMinimal, true, "", nil, nil)
	if opts.EnableThinking {
...
func TestChatTemplateOptions_PropagatesToolSettings(t *testing.T) {
	...
	opts := makeChatTemplateOptions(
		...
		true,
		llmapi.ToolChoiceRequired,
		&parallel,
		nil,
	)
	if opts.ToolChoice != string(llmapi.ToolChoiceRequired) {
		t.Fatalf("ToolChoice = %q, want %q", opts.ToolChoice, llmapi.ToolChoiceRequired)
	}
	if !opts.ParallelToolCalls {
	...
	opts = makeChatTemplateOptions(
		...
		llmapi.ReasoningEffortHigh,
		true,
		llmapi.ToolChoiceNone,
		&parallel,
		nil,
	)
	if opts.ToolChoice != string(llmapi.ToolChoiceNone) {
		t.Fatalf("ToolChoice = %q, want %q", opts.ToolChoice, llmapi.ToolChoiceNone)
	}
	if opts.ParallelToolCalls {
...
func TestChatTemplateOptions_PropagatesResponseFormat(t *testing.T) {
	...
	t.Run("json object", func(t *testing.T) {
		opts := makeChatTemplateOptions(
			...
			"",
			nil,
			&llmapi.ResponseFormat{Type: llmapi.ResponseFormatTypeJSONObject},
		)
	...
	t.Run("json schema", func(t *testing.T) {
		...
		opts := makeChatTemplateOptions(
			...
			"",
			nil,
			&llmapi.ResponseFormat{
				Type:       llmapi.ResponseFormatTypeJSONSchema,
				JSONSchema: &llmapi.ResponseFormatJSONSchema{Schema: json.RawMessage(b)},
			},
		)
...
func TestEmitDone_PrefersToolCallFinishReasonWhenToolCallsExist(t *testing.T) {
	...
	if ev.DoneData.FinishReason != llmapi.FinishReasonToolCall {
		t.Fatalf("FinishReason = %q, want %q", ev.DoneData.FinishReason, llmapi.FinishReasonToolCall)
	}
}
...
func TestBuildChatMessages_ToolRoleRemappedToUser(t *testing.T) {
	msgs := []llmapi.Message{
		...
		{Role: llmapi.RoleTool, Content: "12:34"},
	}
	tools := []llmapi.Tool{{
		Type: llmapi.ToolTypeFunction,
	...
	if got := cmsgs[toolIdx].Role; got != string(llmapi.RoleUser) {
	...
	for _, c := range cmsgs {
		if c.Role == string(llmapi.RoleTool) {
...
func TestBuildChatMessages_InjectsToolPromptIntoExistingSystem(t *testing.T) {
	...
	if cmsgs[0].Role != string(llmapi.RoleSystem) {
...
func TestBuildChatMessages_PrependsSystemWhenToolsAndNoSystem(t *testing.T) {
	...
	if len(cmsgs) < 2 || cmsgs[0].Role != string(llmapi.RoleSystem) {
...
func TestRenderForUpstream_PreservesStructuredToolMessages(t *testing.T) {
	msgs := []llmapi.Message{
		...
		{Role: llmapi.RoleTool, Name: "get_time", Content: "12:34", ToolCallID: "call_1"},
	}
	...
	if got[1].Role != string(llmapi.RoleAssistant) {
	...
	if got[2].Role != string(llmapi.RoleTool) {
...
func TestRenderForUpstream_PreservesImageAsMediaMarker(t *testing.T) {
	msgs := []llmapi.Message{{
		...
		MultiContent: []llmapi.ContentPart{
			{Type: llmapi.ContentPartTypeText, Text: "describe this image:"},
	...
	msgs := []llmapi.Message{
		{
			...
			MultiContent: []llmapi.ContentPart{
				{Type: llmapi.ContentPartTypeText, Text: "hello"},
				{Type: llmapi.ContentPartTypeImageURL, ImageURL: "data:image/png;base64,abc"},
			},
		},
	}
...
// llmapi.ContentPartTypeImageURL, so each malformed shape is something we
...
// llmapi.ResponseFormat into the JSON schema string fed to
...
func TestResponseFormatJSONSchema_Table(t *testing.T) {
	...
	cases := []struct {
		name string
		in   *llmapi.ResponseFormat
		want string
	}{
		{"nil", nil, ""},
		{"json_object", &llmapi.ResponseFormat{Type: llmapi.ResponseFormatTypeJSONObject}, `{"type":"object"}`},
		{
			name: "json_schema_no_payload",
			in:   &llmapi.ResponseFormat{Type: llmapi.ResponseFormatTypeJSONSchema},
			want: "",
		},
		{
		...
		},
		{
			...
			in:   &llmapi.ResponseFormat{Type: llmapi.ResponseFormatTypeText},
			want: "",
		},
	}
	for _, tc := range cases {
...
// mocking the entire llmapi.ResponseFormatJSONSchema type.
...
func TestResponseFormatJSONSchema_MarshalError(t *testing.T) {
	t.Parallel()
	out := responseFormatJSONSchema(&llmapi.ResponseFormat{
		Type: llmapi.ResponseFormatTypeJSONSchema,
		JSONSchema: &llmapi.ResponseFormatJSONSchema{
			Schema: brokenSchema{},
		},
...
func TestConvertTools_Table(t *testing.T) {
	...
	if got := convertTools([]llmapi.Tool{}); got != nil {
...
func TestRenderMessage_Table(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "plain_content",
			msg:  llmapi.Message{Role: llmapi.RoleUser, Content: "hello"},
			want: "hello",
		},
		{
			...
			msg: llmapi.Message{
				...
				Content: "winning",
				MultiContent: []llmapi.ContentPart{
					{Type: llmapi.ContentPartTypeText, Text: "ignored"},
				},
			},
			want: "winning",
		},
		{
			...
			msg: llmapi.Message{
				Role: llmapi.RoleUser,
				MultiContent: []llmapi.ContentPart{
					{Type: llmapi.ContentPartTypeText, Text: "first "},
					{Type: llmapi.ContentPartTypeText, Text: "second"},
				},
			},
			want: "first second",
		},
		...
		{
			name: "empty",
			msg:  llmapi.Message{Role: llmapi.RoleUser},
			want: "",
		},
	}
	for _, tc := range cases {
...
func TestEmitDone_Table(t *testing.T) {
	...
	cases := []struct {
		name        string
		setup       func(*genState)
		wantReason  llmapi.FinishReason
		wantContent string
		wantTools   int
	}{
		{
			...
			wantReason:  llmapi.FinishReasonStop,
			wantContent: "hello",
		},
		{
			...
			wantReason:  llmapi.FinishReasonLength,
			wantContent: "partial",
		},
		{
			...
			setup: func(g *genState) {
				...
				g.finish = llmapi.FinishReasonLength
				g.toolCalls = []llmapi.ToolCall{
					{ID: "c1", Type: llmapi.ToolTypeFunction, Function: llmapi.FunctionCall{Name: "get_time", Arguments: "{}"}},
				}
			},
			wantReason:  llmapi.FinishReasonToolCall,
...
//
// These tests drive llamacpp.Service through the llmapi.Service interface using a
...
// llmapi.Service, backed by the real gemma-4-E2B GGUF stored in testdata/. The
// service is shared across subtests via sync.Once so we pay the ~4.6 GiB load
// cost only once per `go test` invocation.
func loadTestdataService(t *testing.T) llmapi.Service {
	t.Helper()
	...
	// explicit: tests only touch methods that exist on llmapi.Service.
	var svc llmapi.Service = testdataSvc
	return svc
}
...
// observable llmapi.Service contract.
...
func collectCompletion(t *testing.T, ctx context.Context, svc llmapi.Service, req llmapi.Request) completionResult {
	t.Helper()
	it, err := svc.CreateCompletion(ctx, llmapi.ModelEntry{}, req)
	if err != nil {
...
// case drives llmapi.Service.CreateCompletion against the shared real model and
	...
	cases := []testCase{
		{
			...
			req: llmapi.Request{
				Messages: []llmapi.Message{
					{Role: llmapi.RoleUser, Content: "Say hi in a single short word."},
				},
				...
				// emit the answer.
				ReasoningEffort: llmapi.ReasoningEffortMinimal,
				MaxOutputTokens: 16,
			},
		...
		},
		{
			...
			check: func(t *testing.T, res completionResult) {
				...
				if res.Done.FinishReason != llmapi.FinishReasonLength {
					t.Fatalf("FinishReason = %q, want %q", res.Done.FinishReason, llmapi.FinishReasonLength)
				}
			},
		},
		{
			...
			check: func(t *testing.T, res completionResult) {
				...
				if res.Done.FinishReason != llmapi.FinishReasonToolCall {
					t.Fatalf("FinishReason = %q, want %q",
						res.Done.FinishReason, llmapi.FinishReasonToolCall)
				}
				tc := res.ToolCalls[0]
		...
		},
		{
			...
			req: llmapi.Request{
				Messages: []llmapi.Message{
					{Role: llmapi.RoleUser, Content: "Answer in one short sentence: what is 2 + 2?"},
				},
				ReasoningEffort: llmapi.ReasoningEffortMinimal,
				MaxOutputTokens: 32,
			},
...
// end-to-end through llmapi.Service actually accepts an image payload without
		...
		Messages: []llmapi.Message{
			{
				Role: llmapi.RoleUser,
				MultiContent: []llmapi.ContentPart{
					{Type: llmapi.ContentPartTypeText, Text: "Describe the image very briefly."},
					{Type: llmapi.ContentPartTypeImageURL, ImageURL: dataURL},
				},
			},
		},
		MaxOutputTokens: 32,
...
func TestService_Testdata_ContextCancellation(t *testing.T) {
	...
	for {
		...
		if (ev.Type == llmapi.EventTextDelta || ev.Type == llmapi.EventReasoningDelta) && !sawStreamed {
			sawStreamed = true
			cancel()
		}
	}
...
func TestService_Testdata_PrefixCacheReuse(t *testing.T) {
	...
	r1 := collectCompletion(t, ctx, svc, llmapi.Request{
		Messages:        first,
		ReasoningEffort: llmapi.ReasoningEffortMinimal,
		MaxOutputTokens: 8,
	})
	...
	second = append(second,
		llmapi.Message{Role: llmapi.RoleAssistant, Content: r1.Done.Message.Content},
		llmapi.Message{Role: llmapi.RoleUser, Content: "Another, please."},
	)
	r2 := collectCompletion(t, ctx, svc, llmapi.Request{
		Messages:        second,
		ReasoningEffort: llmapi.ReasoningEffortMinimal,
		MaxOutputTokens: 8,
	})
...
// contract through llmapi.Service: turn 1 emits a tool call, we construct a
...
func TestService_Testdata_ToolCallRoundTrip(t *testing.T) {
	...
	turn1 := collectCompletion(t, ctx, svc, llmapi.Request{
		Messages: []llmapi.Message{
			{Role: llmapi.RoleSystem, Content: "You are a helpful assistant. Prefer calling tools when available."},
			{Role: llmapi.RoleUser, Content: "What time is it right now? Call the tool."},
		},
	...
	turn2 := collectCompletion(t, ctx, svc, llmapi.Request{
		Messages: []llmapi.Message{
			{Role: llmapi.RoleSystem, Content: "You are a helpful assistant. Prefer calling tools when available."},
			{Role: llmapi.RoleUser, Content: "What time is it right now? Call the tool."},
			{
...
func TestService_Testdata_ReasoningCaptured(t *testing.T) {
	...
	res := collectCompletion(t, ctx, svc, llmapi.Request{
		Messages: []llmapi.Message{
			{Role: llmapi.RoleUser, Content: "What is 2 + 2? Think step by step then answer."},
		},
...
func TestService_Testdata_SerializesConcurrentCreateCompletion(t *testing.T) {
	...
	req := llmapi.Request{
		Messages: []llmapi.Message{
			{Role: llmapi.RoleUser, Content: "Reply with a single short word."},
		},
		ReasoningEffort: llmapi.ReasoningEffortMinimal,
		MaxOutputTokens: 8,
	}
...
func TestService_Testdata_FinishReasonStop(t *testing.T) {
	...
	res := collectCompletion(t, ctx, svc, llmapi.Request{
		Messages: []llmapi.Message{
			{Role: llmapi.RoleUser, Content: "Reply with exactly the word: ok"},
		},
		ReasoningEffort: llmapi.ReasoningEffortMinimal,
		MaxOutputTokens: 64,
	})
	...
	if res.Done.FinishReason != llmapi.FinishReasonStop {
		t.Fatalf("FinishReason = %q, want %q (content=%q)",
			res.Done.FinishReason, llmapi.FinishReasonStop, res.Done.Message.Content)
	}
	if res.Text == "" {
...
func TestService_Testdata_CountTokens_ShapeGrowsWithPrompt(t *testing.T) {
	svc := loadTestdataService(t)
	short, err := svc.CountTokens(llmapi.ModelEntry{}, []llmapi.Message{{Role: llmapi.RoleUser, Content: "hi"}})
	if err != nil {
		t.Fatalf("CountTokens short: %v", err)
	}
	long, err := svc.CountTokens(llmapi.ModelEntry{}, []llmapi.Message{
		{Role: llmapi.RoleUser, Content: strings.Repeat("hello world ", 64)},
	})
	if err != nil {
...
// blows past that. The error must satisfy *llmapi.ErrContextWindowExceeded
...
func TestService_Testdata_ContextWindowExceeded(t *testing.T) {
	...
	_, err := svc.CreateCompletion(ctx, llmapi.ModelEntry{}, llmapi.Request{
		Messages: []llmapi.Message{
			{Role: llmapi.RoleUser, Content: huge},
		},
		ReasoningEffort: llmapi.ReasoningEffortMinimal,
		MaxOutputTokens: 8,
	})
	...
	var cwe *llmapi.ErrContextWindowExceeded
	if !errors.As(err, &cwe) {
		t.Fatalf("error type = %T (%v), want *llmapi.ErrContextWindowExceeded", err, err)
	}
...
func TestService_Testdata_ResponseFormat_JSONObject(t *testing.T) {
	...
	res := collectCompletion(t, ctx, svc, llmapi.Request{
		Messages: []llmapi.Message{
			{Role: llmapi.RoleSystem, Content: "Reply only with JSON."},
			{Role: llmapi.RoleUser, Content: `Return a JSON object with one key "answer" whose value is the string "ok".`},
		},
		ReasoningEffort: llmapi.ReasoningEffortMinimal,
...
func TestService_Testdata_StopStringsHonoured(t *testing.T) {
	...
	res := collectCompletion(t, ctx, svc, llmapi.Request{
		Messages: []llmapi.Message{
			{Role: llmapi.RoleUser, Content: "Reply with: done."},
		},
		ReasoningEffort: llmapi.ReasoningEffortMinimal,
		MaxOutputTokens: 64,
	})
