...
func buildToolSystemPrompt(tools []llmapi.Tool) (string, error) {
	if len(tools) == 0 {
		return "", nil
	}
	...
	for _, t := range tools {
		...
		if e.Type == "" {
			e.Type = string(llmapi.ToolTypeFunction)
		}
...
func renderAssistantWithToolCalls(msg llmapi.Message) (string, error) {
...
// during the chunk.
func (p *toolCallParser) Feed(chunk string) (string, []llmapi.ToolCall, error) {
	if chunk == "" {
		return "", nil, nil
	}
...
func (p *toolCallParser) Flush() (string, []llmapi.ToolCall, error) {
	s := p.buf.String()
	p.buf.Reset()
	if p.inCall {
...
// parseToolCall decodes a single <tool_call> body into an llmapi.ToolCall.
func parseToolCall(body string) (llmapi.ToolCall, error) {
	var raw toolCallJSON
	if err := json.Unmarshal([]byte(body), &raw); err != nil {
		return llmapi.ToolCall{}, fmt.Errorf("llamacpp: tool_call json: %w (body=%q)", err, body)
	}
	if raw.Name == "" {
		return llmapi.ToolCall{}, fmt.Errorf("llamacpp: tool_call missing name (body=%q)", body)
	}
