...
func feedAll(t *testing.T, chunks []string) (string, []llmapi.ToolCall, error) {
	t.Helper()
	var p toolCallParser
	var text strings.Builder
	var calls []llmapi.ToolCall
...
func TestToolCallParser_SingleCallInOneChunk(t *testing.T) {
	...
	if calls[0].Type != llmapi.ToolTypeFunction {
...
func TestRenderAssistantWithToolCalls_NoCalls(t *testing.T) {
	msg := llmapi.Message{Role: llmapi.RoleAssistant, Content: "just text"}
	out, err := renderAssistantWithToolCalls(msg)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
