...
func Grantee() (plugin.Grantee, []plugin.Permission) {
	webcamGrantee, perms := plugutil.NewCommandSplitHandler(plugutil.CommandSplitHandlerConfig{
		...
		Handler: func(cmd textapi.Command, grants []plugin.Grant, broker proto.MuxBroker,
			...
			for _, grant := range grants {
				if grant.Permission == plugin.PermissionBrowserEventPublisher {
					var err error
					publisher, err = browserplugin.EventPublisher(grant, broker)
					if err != nil {
						return nil, fmt.Errorf("browser plugin event publisher: %v", err)
					}
				}
			}
	...
	convertImageGrantee, _ := plugutil.NewCommandSplitHandler(plugutil.CommandSplitHandlerConfig{
		SplitOrientation: browserapi.OrientationRight,
		Handler: func(cmd textapi.Command, grants []plugin.Grant, broker proto.MuxBroker,
	...
	perms = append(perms, plugin.PermissionBrowserEventPublisher)
	grantee := plugutil.MultiGrantee(webcamGrantee, convertImageGrantee)
