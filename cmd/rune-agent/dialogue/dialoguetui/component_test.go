...
// TestCancelPromptInputReturnsToSelection verifies that CancelPromptInput
// removes the text input box and re-inserts the selection, allowing the
// user to pick a different option. This exercises the fix for the bug where
// pressing Esc in prompt input mode dismissed the entire prompt.
func TestCancelPromptInputReturnsToSelection(t *testing.T) {
	comp := NewComponent(ComponentConfig{})
	comp.Resize(20, 10)
	...
	tests := []comptest.TestCase{
		{
			// Show prompt with a RequiresInput option.
			Action: func() {
				comp.AddPrompt("Review?", "", "", []PromptEventOption{
					{Label: "Approve"},
					{Label: "Feedback", RequiresInput: true},
				}, false, resultCh)
			},
			Expected: "Review?              \n" +
				"                     \n" +
		...
		{
			// Move cursor to "Feedback" and enter input mode.
			Action: func() {
				comp.PromptMoveDown()
				ok := comp.StartPromptInput()
				assert.True(t, ok, "StartPromptInput should succeed")
				assert.True(t, comp.PromptInputMode(), "should be in input mode")
			},
			// Input box replaces the selection.
			Expected: "Type your feedback   \n" +
				"                     \n" +
				"                     \n" +
				"                     \n" +
				"                     \n" +
		...
		{
			// Cancel input — should return to selection.
			Action: func() {
				ok := comp.CancelPromptInput()
				assert.True(t, ok, "CancelPromptInput should succeed")
				assert.False(t, comp.PromptInputMode(), "should not be in input mode")
				assert.True(t, comp.HasActivePrompt(), "prompt should still be active")
			},
			Expected: "Review?              \n" +
				"                     \n" +
				"  Approve            \n" +
...
func TestComponentAnimationPersistsDuringToolCall(t *testing.T) {
	comp := NewComponent(ComponentConfig{})
	comp.Resize(20, 10)
	...
	tests := []comptest.TestCase{
		...
		{
			Action: func() {
				comp.AddReceiveMessageChunk("Done!")
			},
			Expected: "✓ read_file          \nok                   \nDone!                \n                     \n...                  \n                     \n                     \n ┌──────────────┐    \n │              │    \n └──────────────┘    \n                     ",
		},
	}
...
// TestComponentHintPersistsThroughAgentTurn verifies that the receive-message
// hint stays visible across a full agentic turn: text streaming, tool calls,
// tool results, message breaks, and the next LLM response. Only an explicit
// RemoveReceiveMessageHint call should remove it.
func TestComponentHintPersistsThroughAgentTurn(t *testing.T) {
	comp := NewComponent(ComponentConfig{})
	comp.Resize(20, 10)
	...
	tests := []comptest.TestCase{
		...
		{
			// LLM streams text — hint stays at back
			Action: func() {
		...
		{
			// LLM calls a tool — hint moves after tool entry
			Action: func() {
				comp.AddToolCall("t1", "search", "", "")
			},
			Expected: "Thinking             \n" +
				"                     \n" +
				"⚙ search             \n" +
		...
		{
			// Tool completes — hint stays after result
			Action: func() {
				comp.CompleteToolCall("t1", "search", "", "", "found", false)
			},
			Expected: "Thinking             \n" +
				"                     \n" +
				"✓ search             \n" +
		...
		{
			// New LLM turn: break + next text chunk — hint still present
			Action: func() {
				comp.AddReceiveMessageBreak()
				comp.AddReceiveMessageChunk("Result")
			},
			Expected: "Thinking             \n" +
				"                     \n" +
				"✓ search             \n" +
		...
		{
			// Explicit remove — hint disappears
			Action: func() {
				comp.RemoveReceiveMessageHint()
			},
			Expected: "Thinking             \n" +
				"                     \n" +
				"✓ search             \n" +
...
func TestComponentHintPersistsThroughReasoning(t *testing.T) {
	comp := NewComponent(ComponentConfig{})
	comp.Resize(20, 10)
	...
	tests := []comptest.TestCase{
		...
		{
			// Reasoning chunk — hint stays after annotation
			Action: func() {
...
func TestComponentHintPersistsThroughError(t *testing.T) {
	comp := NewComponent(ComponentConfig{})
	comp.Resize(20, 10)
...
func TestComponentHintPersistsThroughSendMessage(t *testing.T) {
	comp := NewComponent(ComponentConfig{})
	comp.Resize(20, 10)
	...
	tests := []comptest.TestCase{
		...
		{
			// User sends a message (e.g. slash command) during live turn — hint stays at back
			Action: func() {
...
func TestComponentHintRestoredAfterPromptSelect(t *testing.T) {
	comp := NewComponent(ComponentConfig{})
	comp.Resize(20, 10)
	...
	tests := []comptest.TestCase{
		...
		{
			// Prompt replaces hint — hint saved and hidden.
			Action: func() {
				comp.AddPrompt("OK?", "", "", []PromptEventOption{
					{Label: "Yes"}, {Label: "No"},
				}, false, resultCh)
			},
			Expected: "OK?                  \n" +
		...
		{
			// Select first option — prompt removed, hint restored.
			Action: func() {
				ch, vals := comp.PreparePromptSelect()
				if ch != nil {
					ch <- vals
				}
			},
		...
		{
			// Subsequent streaming works — hint moves to back.
			Action: func() {
				comp.AddReceiveMessageChunk("Done")
			},
			Expected: "Done                 \n" +
...
// TestResetPreservingHintKeepsHintAfterCompaction verifies that
// ResetPreservingHint clears all messages but keeps the receive-message
// hint visible — this is the expected behavior during auto-compaction.
func TestResetPreservingHintKeepsHintAfterCompaction(t *testing.T) {
	comp := NewComponent(ComponentConfig{})
	comp.Resize(20, 10)
	...
	tests := []comptest.TestCase{
		{
			...
			Action: func() {
				...
				comp.AddReceiveMessageChunk("Hello")
				comp.AddToolCall("t1", "search", "", "")
				comp.CompleteToolCall("t1", "search", "", "", "ok", false)
			},
			Expected: "Hello                \n" +
				"                     \n" +
				"✓ search             \n" +
		...
		{
			// ResetPreservingHint clears content but hint stays.
			Action: func() {
				comp.ResetPreservingHint()
			},
			Expected: "$                    \n" +
				"                     \n" +
				"                     \n" +
		...
		{
			// New content after compaction — hint moves to back.
			Action: func() {
				comp.AddReceiveMessage("Summary")
			},
			Expected: "Summary              \n" +
				"                     \n" +
				"$                    \n" +
...
// TestComponentHintRestoredAfterPromptDismiss verifies that the hint is
// also restored when the user dismisses (Esc) a prompt rather than selecting.
func TestComponentHintRestoredAfterPromptDismiss(t *testing.T) {
	comp := NewComponent(ComponentConfig{})
	comp.Resize(20, 10)
	...
	tests := []comptest.TestCase{
		...
		{
			// Dismiss — prompt removed, hint restored.
			Action: func() {
				ch := comp.PreparePromptDismiss()
				if ch != nil {
					ch <- nil
				}
			},
