...
// blob is missing.
func (r *Registry) Get(_ context.Context, model string) (llmregistry.ModelEntry, bool) {
...
func (r *Registry) scan() []llmregistry.ModelEntry {
	root := filepath.Join(r.cache.Root(), "manifests")
	info, err := os.Stat(root)
	if err != nil || !info.IsDir() {
		return nil
	}
	var out []llmregistry.ModelEntry
	_ = filepath.WalkDir(root, func(path string, d os.DirEntry, walkErr error) error {
		...
		out = append(out, llmregistry.ModelEntry{
	...
	slices.SortFunc(out, func(a, b llmregistry.ModelEntry) int {
...
// Compile-time confirmation that Registry satisfies the registry
// interface.
var _ llmregistry.Registry = (*Registry)(nil)
