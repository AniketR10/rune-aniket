...
// Service implements llm.Service on top of a locally-loaded GGUF model.
//
...
func (s *Service) CountTokens(msgs []llm.Message) (int, error) {
...
func responseFormatJSONSchema(format *llm.ResponseFormat) string {
	if format == nil {
		return ""
	}
	switch format.Type {
...
// renderForUpstream adapts []llm.Message into the C-side []ChatMessage
...
// prompt evaluation.
func renderForUpstream(msgs []llm.Message) ([]ChatMessage, [][]byte) {
...
// convertTools translates llm.Tool definitions into the
...
// buildChatMessages converts a slice of llm.Messages into the ChatMessage
...
func buildChatMessages(msgs []llm.Message, tools []llm.Tool) ([]ChatMessage, error) {
...
// single llm.Message. Multi-content parts are folded to their text components
// (images are dropped), and assistant tool calls are inlined as <tool_call>
// blocks so the model sees its prior decisions.
func renderMessage(m llm.Message) (string, error) {
...
func (s *Service) CreateCompletion(
	...
	if !hasMedia && len(tokens) > s.context.NCtx() {
		...
		return nil, &llm.ErrContextWindowExceeded{Count: len(tokens), Max: s.context.NCtx()}
	}
	...
	started := time.Now()
	usage := llm.Usage{TokensSent: len(tokens)}
	tokensCached := nPast
...
func (g *genState) next(ctx context.Context) (llm.Event, bool, error) {
	...
	if err := ctx.Err(); err != nil {
		g.done = true
		g.finish = llm.FinishReasonStop
		return llm.Event{Type: llm.EventStreamError, Error: err}, true, nil
	}
...
func (g *genState) ingestPiece(ctx context.Context, piece string) (llm.Event, bool, error) {
	if piece != "" {
		...
		if stop {
			...
			if piece != "" {
				if g.upstreamStream != nil {
					...
					g.pending = append(g.pending, llm.Event{Type: llm.EventTextDelta, Text: piece})
				} else {
					...
					if text != "" {
						...
						g.pending = append(g.pending, llm.Event{Type: llm.EventTextDelta, Text: text})
					}
					for i := range calls {
						tc := calls[i]
						g.toolCalls = append(g.toolCalls, tc)
						g.pending = append(g.pending, llm.Event{Type: llm.EventToolCallDone, ToolCall: &tc})
					}
				}
			}
			if len(g.pending) > 0 {
	...
	if !g.hasTools {
		g.messageBuf.WriteString(piece)
		return llm.Event{Type: llm.EventTextDelta, Text: piece}, true, nil
	}
	...
	for i := range calls {
		tc := calls[i]
		g.toolCalls = append(g.toolCalls, tc)
		g.pending = append(g.pending, llm.Event{
...
// providers' contract.
func (g *genState) ingestPieceUpstream(ctx context.Context, piece string) (llm.Event, bool, error) {
...
// maps it onto llm.Event. Tool-call deltas update g.toolCalls in place;
...
func (g *genState) drainUpstream() {
	for {
		...
		case ChatStreamEventTextDelta:
			...
			g.pending = append(g.pending, llm.Event{Type: llm.EventTextDelta, Text: ev.Text})
...
func (g *genState) recordToolCallDelta(ev ChatStreamEvent) {
	...
	for len(g.toolCalls) <= idx {
		g.toolCalls = append(g.toolCalls, llm.ToolCall{Type: llm.ToolTypeFunction})
	}
	tc := &g.toolCalls[idx]
	if ev.ToolID != "" {
		tc.ID = ev.ToolID
...
func (g *genState) finalize() (llm.Event, bool, error) {
	if flushed := g.flushPendingStop(); flushed != "" {
		if g.upstreamStream != nil {
			...
			g.pending = append(g.pending, llm.Event{Type: llm.EventTextDelta, Text: flushed})
		}
	}
	if g.upstreamStream != nil {
...
func (g *genState) emitDone() (llm.Event, bool, error) {
	if g.closed {
...
func (g *genState) close() error {
	...
	if g.closed && g.finish != "" && g.finish != llm.FinishReasonNull && !g.hasMedia {
...
// Ensure Service satisfies llm.Service at compile time.
