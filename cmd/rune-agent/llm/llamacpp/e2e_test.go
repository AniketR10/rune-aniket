...
// llm.Service interface against a real, small GGUF model. The model is
...
func TestE2E_CreateCompletion_Basic(t *testing.T) {
	...
	it, err := svc.CreateCompletion(ctx, llm.Request{
		Messages: []llm.Message{
			{Role: llm.RoleUser, Content: "Say hi in one word."},
		},
		MaxOutputTokens: 8,
	})
	if err != nil {
...
func TestE2E_CreateCompletion_Length(t *testing.T) {
	...
	for _, e := range evs {
		if e.Type == llm.EventStreamDone {
			if e.DoneData.FinishReason != llm.FinishReasonLength {
				t.Fatalf("finish reason = %q, want %q",
					e.DoneData.FinishReason, llm.FinishReasonLength)
			}
			return
		}
	}
...
func TestE2E_CreateCompletion_Tools(t *testing.T) {
	...
	if done.FinishReason != llm.FinishReasonToolCall {
		t.Fatalf("finish reason = %q, want %q", done.FinishReason, llm.FinishReasonToolCall)
	}
	tc := toolCalls[0]
...
func TestE2E_PrefixCacheReuse(t *testing.T) {
	...
	run := func(msgs []llm.Message) *llm.DoneData {
		t.Helper()
		it, err := svc.CreateCompletion(ctx, llm.Request{
			Messages:        msgs,
			MaxOutputTokens: 8,
	...
	first := []llm.Message{
		{Role: llm.RoleSystem, Content: "You are a terse assistant that answers in one short sentence."},
	...
	second = append(second,
		llm.Message{Role: llm.RoleAssistant, Content: d1.Message.Content},
...
func TestE2E_ContextCancellation(t *testing.T) {
	...
	for {
		...
		if ev.Type == llm.EventTextDelta && !sawDelta {
			sawDelta = true
			cancel()
		}
	}
