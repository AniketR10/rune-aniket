...
func (t *Terminal) sgrSequenceHandler(params []string) bool {
	// log.Tracef("Terminal.SGR: %v", params)
	...
	for i := range params {
		...
		case "38":
			...
			return false
		case "39":
			t.setCursorFgColor(t.theme.Default.Fg)
		case "48":
			color, _ := t.theme.ColourFromAnsi(params[i+1:], true)
			t.setCursorBgColor(color)
			return false
		case "49":
			t.setCursorBgColor(t.theme.Default.Bg)
		case "08", "8", "9", "09", "10", "11", "12", "13", "14", "15", "16", "18", "19",
			"23", "26", "28", "29", "50", "51", "52", "53", "54", "55", "58", "59", "60",
		...
		default:
			...
			case i >= 30 && i <= 37, i >= 90 && i <= 97:
				t.setCursorFgColor(t.theme.ColourFrom4Bit(i))
			case i >= 40 && i <= 47, i >= 100 && i <= 107:
				t.setCursorBgColor(t.theme.ColourFrom4Bit(i))
			}
	...
	y := buffer.CursorLine()
	if cell := buffer.GetCell(x, y); cell != nil {
