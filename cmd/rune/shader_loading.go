...
const loadingShaderFPS = 30

// loadingShader returns the shader played while a workspace is
// loading. The runtime caps it at a fixed visual budget; if the
// workspace finishes loading earlier, the runner replaces it
// with the open shader.
func loadingShader(defaultAttr term.Attributes) shader.Shader {
	return timeshader.PingPong(
		glslshader.RisingChars(glslshader.DefaultRisingCharsParams(), defaultAttr))
}
