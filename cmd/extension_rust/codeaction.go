...
func (h *codeActionCmd) HandleCommand(
	...
	// An empty kind lists every assist applicable at the cursor. rust-analyzer
	// groups many assists under a handful of broad kinds (refactor.rewrite,
	// refactor.extract, ...), so a nil Only filter is how the user browses the
	// full menu rather than pre-selecting a single kind.
