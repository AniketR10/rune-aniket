...
: "${BLUECTL_CONFIG_DIR:?BLUECTL_CONFIG_DIR is not set. Use the runectl-{prod,staging}-dist* make targets so the bluectl env+os+arch is selected by the target.}"
BLUE_RELEASE_TAR="${BLUE_RELEASE_TAR:-runectl.tar.gz}"
...
blue_release_dist() {
	...
	"${BLUE_EXEC[@]}" release upload \
		...
		-y \
		-k $BLUE_PGP_KEY \
		-r $BLUE_PGP_KEYRING runectl $BLUE_RELEASE_TAG $BLUE_RELEASE_TAR
}
