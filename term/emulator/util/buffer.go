...
type Buffer struct {
	...
	savedCursorAttr       *term.Attributes
...
func (buffer *Buffer) restoreCursor() {
	if buffer.savedCursorAttr != nil {
		...
		copiedAttr.Bg = buffer.defaultCell(false).Bg
		copiedAttr.Fg = buffer.defaultCell(false).Fg
...
func (buffer *Buffer) getCursorAttr() *term.Attributes {
	return &buffer.cursorAttr
}

func (buffer *Buffer) GetCell(viewCol uint16, viewRow uint16) *term.Cell {
	rawLine := buffer.convertViewLineToRawLine(viewRow)
	return buffer.getRawCell(viewCol, rawLine)
}

func (buffer *Buffer) getRawCell(viewCol uint16, rawLine uint64) *term.Cell {
...
func (buffer *Buffer) insertBlankCharacters(count int) {
	...
	for i := 0; i < count; i++ {
		...
		buffer.lines[index].cells = append(cells[:buffer.cursorPosition.Col], append([]term.Cell{buffer.defaultCell(true)}, cells[buffer.cursorPosition.Col:]...)...)
	}
}
...
func (buffer *Buffer) write(runes ...MeasuredRune) {
	...
	for _, r := range runes {
		// log.Tracef("(%p).Buffer.Write: '%s' -> %v", buffer, string(r.Rune), buffer.cursorAttr)
...
func (buffer *Buffer) eraseDisplay() {
	for i := uint16(0); i < (buffer.ViewHeight()); i++ {
		...
		if int(rawLine) < len(buffer.lines) {
			buffer.lines[int(rawLine)].cells = []term.Cell{}
		}
	}
}
