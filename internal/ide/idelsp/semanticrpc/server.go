...
func (s *Server) WorkspaceDiagnostic(
	...
	for i, prev := range prevProtos {
		prevIDs[i] = semanticapi.PreviousResultID{
			URI:   prev.GetUri(),
			Value: prev.GetValue(),
		}
	}
	...
	items := make([]*semanticrpc.WorkspaceDocumentDiagnosticReport, len(result.Items))
	for i, item := range result.Items {
		items[i] = &semanticrpc.WorkspaceDocumentDiagnosticReport{
			Kind:     item.Kind,
			ResultId: item.ResultID,
			Uri:      item.URI,
			Version:  item.Version,
			Items:    semanticrpc.DiagnosticsToProto(item.Items),
		}
	}
	return &semanticrpc.WorkspaceDiagnosticResponse{Items: items}, nil
}
