...
// TODO allow creates, but delegate to in-memory scheme (allow interop with swp files):
// - when open is called, if file in in-memory scheme, then do one thing, otherwise do the real thing.
// - allow writes to file and implement sync by checking if file IS NOT in-memory first, if it's not then ignore.
// - implement rename but that only works if updating real file from in-memory file.
// - implement remove of in-memory created swp files.
func (s *readScheme[T]) Open(path string, flag int, perm os.FileMode) (
	...
	if flag&os.O_EXCL != 0 || flag&os.O_CREATE != 0 || flag&os.O_TRUNC != 0 ||
	...
	return readFile{File: workspace.NewMemoryFile(path, perm, data)}, nil
}
